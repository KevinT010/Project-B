...
public class AccountRegistrationLogic
{
    ...
    {
        if ((phoneNumber.StartsWith("0") || phoneNumber.StartsWith("+")) && phoneNumber.Length >= 5 && phoneNumber.Length <= 15)
        {
            try
            {
    ...
    public bool AccountRegistrationValidation(string firstName, string lastName, string email, string phoneNumber, string password)
    {
        ...
        {
            _access.InsertAccount(new AccountModel(firstName, lastName, email, phoneNumber, BCrypt.Net.BCrypt.HashPassword(password), 1, 0, 0));
            return true;
        }
        else
        {
